...
<template>
    <div ref="containerRef" class="relative inline-block text-left font-sans select-none z-10">
        ...
        <Transition enter-active-class="transition duration-200 ease-out"
            ...
            <div v-if="isOpen"
                class="absolute top-full left-0 mt-2 w-32 max-h-60 backdrop-blur-xl border border-white/10 rounded-2xl shadow-xl overflow-hidden flex flex-col">
                <div ref="listRef" class="overflow-y-auto scrollbar-hide py-1">
                    <button v-for="year in useData" :key="year" :data-active="year === modelValue" @click="select(year)"
                        class="w-full text-left px-4 py-2.5 text-sm transition-colors relative flex items-center justify-between group"
                        :class="year === modelValue ? 'text-white bg-white/5' : 'text-zinc-500 hover:text-zinc-300'">
