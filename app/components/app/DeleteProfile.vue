<script setup lang="ts">
            ...
            >
              ...
              >
                ...
                <div class="relative z-10 flex flex-col items-center text-center">
                  ...
                  <div class="w-16 h-16 rounded-full bg-red-500/10 flex items-center justify-center mb-6 ring-1 ring-red-500/30">
                  ...
                  <div class="w-full relative">
                    <div :class="{ 'animate-shake': isHolding && progress > 50 }">
                      <button 
                        @mousedown="startHold"
                        @mouseup="stopHold"
                        @mouseleave="stopHold"
                      ...
                      >
                        <div 
                          class="absolute bottom-0 left-0 h-full bg-red-600 transition-all duration-75 ease-linear"
                          :style="{ width: `${progress}%` }"
                        ></div>
    
                        <div class="relative z-10 flex items-center gap-2 pointer-events-none">
                          <component 
                            :is="isDeleted ? Skull : Trash2" 
                            class="w-5 h-5 transition-colors"
                            :class="progress > 50 ? 'text-white' : 'text-red-500'" 
                          />
                          <span 
                            class="font-bold uppercase tracking-widest text-sm transition-colors"
                            :class="progress > 50 ? 'text-white' : 'text-red-500'"
                          >
                            {{ isDeleted ? 'Terminated' : (isHolding ? 'Hold to Delete...' : 'Press & Hold') }}
                          </span>
                        </div>
                      </button>
                    </div>
    
                    <p v-if="!isDeleted" class="mt-4 text-[10px] text-zinc-600 font-bold uppercase tracking-widest opacity-60">
                      Release to cancel
                    </p>
                  </div>
                ...
                <div class="absolute -bottom-20 -left-20 w-64 h-64 bg-red-600/10 blur-[80px] pointer-events-none rounded-full"></div>
    ...
    <style scoped>
    /* Shake Animation for high tension */
    @keyframes shake {
