<script setup lang="ts">
  ...
  // --- Props ---
  interface Props {
    initialWeight?: number; // e.g., Yesterday's weight
    targetWeight?: number;
  }
  ...
  <template>
    <div class="flex flex-col items-center justify-center w-full max-w-sm mx-auto p-8 font-sans">
      ...
      <div class="relative w-full flex flex-col items-center justify-center mb-10 group">
        ...
        <div 
          class="relative z-10 flex items-baseline gap-1 cursor-pointer"
          @click="toggleInput"
        >
          <template v-if="!isCustomInput">
            <span class="text-7xl font-bold text-white tracking-tighter tabular-nums transition-all duration-200 hover:scale-105 hover:text-indigo-200">
          ...
          <div v-else class="relative w-48">
            <input 
              ...
              @keydown.enter="isCustomInput = false"
            />
          </div>
        </div>
      ...
      <div class="w-full relative h-24">
        ...
        <transition name="slide-up" mode="out-in">
          ...
          <div v-else class="flex flex-col items-center justify-center w-full h-full">
            ...
            <button @click="hasSaved = false" class="text-neutral-600 text-xs mt-2 hover:text-white underline decoration-neutral-700 underline-offset-4">
              Undo
            </button>
          </div>
