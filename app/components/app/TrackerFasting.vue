...
const SOAK_GOAL_SECONDS = GOAL_DURATION_HOURS * 60 * 60; // 16 hours (57600 seconds)
...
const logFastSession = async () => {
  pauseTimer();
  if (elapsedTime.value < 60000) { // Fast must be at least 1 minute long
    saveMessage.value = 'Session too short. Must be at least 1 minute.';
    setTimeout(() => saveMessage.value = null, 3000);
    return;
  }

  isSaving.value = true;
  saveMessage.value = 'Logging fast session...';
  ...
  const payload = {
    ...
    // Total duration of the fast in seconds
  ...
  try {
    ...
    // Replace with actual $fetch call to your /api/save-fast endpoint.
    // ----------------------------------------------------------------------
    ...
    saveMessage.value = `Fast of ${currentSoakTimeFormatted.value} logged successfully!`;
    resetTimer();
  } catch (error) {
    console.error('Failed to save fast session:', error);
...
<template>
  <!-- Custom Fasting Gradient Background (Purples and Vitality Orange) -->
  <div ref="trackerRef" 
  ...
  >
    ...
    <div
      class="w-full max-w-xl rounded-3xl bg-white/5 p-6 sm:p-10 shadow-2xl backdrop-blur-lg space-y-8 
             border border-purple-400/20 ring-4 ring-purple-500/20 transition-all duration-500 hover:ring-purple-400/30"
    >
      
      <!-- Header -->
      <header class="text-center" v-motion="{ initial: { opacity: 0, y: -20 }, enter: { opacity: 1, y: 0, transition: { delay: 0.1 } } }">
        <h1 class="text-4xl font-extrabold flex items-center justify-center bg-clip-text text-transparent bg-gradient-to-r from-purple-300 to-orange-400">
          <!-- Hourglass Icon (Fasting/Time) -->
          <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="w-8 h-8 mr-3 text-orange-400">
            <path d="M5 2a1 1 0 0 1 1-1h12a1 1 0 0 1 1 1v2a1 1 0 0 1-1 1H6a1 1 0 0 1-1-1z"/>
            <path d="M5 22a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2a1 1 0 0 0-1-1H6a1 1 0 0 0-1 1z"/>
            <path d="M8 5v4a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V5"/>
            <path d="M8 19v-4a1 1 0 0 1 1-1h6a1 1 0 0 1 1 1v4"/>
            <path d="m14 10-2 2-2-2"/>
            <path d="m10 14 2-2 2 2"/>
          </svg>
          Fasting Tracker
        </h1>
        <p class="mt-2 text-gray-300">Embrace the metabolic shift. Target: {{ GOAL_DURATION_HOURS }} hours.</p>
      </header>
      ...
      >
        ...
        <div class="w-full bg-gray-700 rounded-full h-3 overflow-hidden shadow-inner shadow-black/50">
          <div
            ...
            :class="[isGoalReached ? 'bg-orange-500' : 'bg-purple-500']"
            class="h-3 rounded-full transition-all duration-700 ease-out"
          ></div>
        </div>
        
        <p class="text-sm text-gray-400 font-medium pt-2">
          {{ progressPercent.toFixed(0) }}% towards {{ GOAL_DURATION_HOURS }} hr Goal
          <span v-if="isGoalReached" class="text-orange-400 font-bold"> - Goal Surpassed!</span>
        </p>
      </div>
      ...
      <div class="pt-4 text-center text-gray-500 text-sm" v-motion="{ initial: { opacity: 0 }, enter: { opacity: 1, transition: { delay: 0.9 } } }">
        <p>Fast duration is logged in seconds for long-term analysis.</p>
      </div>
