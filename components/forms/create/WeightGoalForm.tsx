...
import { addGoalWeight } from "@/actions/sections/body/weight";
...
export default function WeightGoalForm() {
    ...
    const handleSubmit = async (formData: FormData) => {
        try {
            ...
            await addGoalWeight({
                goal_weight: formData.get("weight")
            });
        ...
        <form ref={ref} action={handleSubmit}>
            <div className="space-y-12 px-[2rem]">
                <div className="border-b border-gray-900/10 pb-12">
                    <h2 className="text-base/7 font-semibold">Goal Weight Information</h2>
                    <p className="mt-1 text-sm/6 text-gray-600">Insert your goal weight.</p>
                    {personInfo}
                </div>
            </div>
