import { _ as __nuxt_component_0$1, a as __nuxt_component_0 } from './Icon-A8r6hdbA.mjs';
import { defineComponent, ref, computed, resolveDirective, mergeProps, useSSRContext } from 'vue';
import { ssrRenderComponent, ssrRenderAttrs, ssrInterpolate, ssrGetDirectiveProps, ssrRenderStyle, ssrRenderClass, ssrRenderAttr, ssrIncludeBooleanAttr } from 'vue/server-renderer';
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (isRunning.value) {
        ...
        _push(ssrRenderComponent(_component_baseIcon, {
          iconName: "material-symbols:pause",
          size: "40"
        }, null, _parent));
        _push(`</span>`);
      } else {
        _push(`<span>`);
        _push(ssrRenderComponent(_component_baseIcon, {
          iconName: "material-symbols:play-arrow-outline-rounded",
          size: "40"
        }, null, _parent));
        _push(`</span>`);
      }
      _push(`</button><button${ssrIncludeBooleanAttr(isSaving.value || elapsedTime.value === 0 || isRunning.value) ? " disabled" : ""} class="${ssrRenderClass([{ "opacity-50": isSaving.value || elapsedTime.value === 0 || isRunning.value }, "p-4 rounded-full bg-green-600 font-semibold text-white transition duration-300 transform active:scale-[0.9] shadow-xl shadow-green-700/50 hover:bg-green-700 focus:outline-none focus:ring-4 focus:ring-green-500 focus:ring-offset-2 focus:ring-offset-gray-900 disabled:opacity-50 disabled:cursor-not-allowed"])}">`);
      _push(ssrRenderComponent(_component_baseIcon, {
        iconName: "material-symbols:save-outline-rounded",
        size: "40"
      }, null, _parent));
      _push(`</button><button${ssrIncludeBooleanAttr(isSaving.value || isRunning.value) ? " disabled" : ""} class="${ssrRenderClass([{ "opacity-50": isSaving.value || isRunning.value }, "p-4 rounded-full bg-red-600 font-semibold text-white hover:bg-red-700 transition duration-300 transform active:scale-[0.9] shadow-xl shadow-red-700/50 focus:outline-none focus:ring-4 focus:ring-red-500 focus:ring-offset-2 focus:ring-offset-gray-900 disabled:opacity-50 disabled:cursor-not-allowed"])}">`);
      _push(ssrRenderComponent(_component_baseIcon, {
        iconName: "material-symbols:reset-shadow-outline-rounded",
        size: "40"
      }, null, _parent));
      _push(`</button></div><div${ssrRenderAttrs(mergeProps({ class: "pt-4 text-center text-gray-500 text-sm" }, ssrGetDirectiveProps(_ctx, _directive_motion, { initial: { opacity: 0 }, enter: { opacity: 1, transition: { delay: 0.9 } } })))}><p>Fast duration is logged in seconds for long-term analysis.</p></div></div></div>`);
    };
  }
});
...
//# sourceMappingURL=index-CpWfMUBN.mjs.map
